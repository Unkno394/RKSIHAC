...
const Prism: React.FC<PrismProps> = ({
  ...
  transparent = true,
  scale = 3.6,
  hueShift = 0, // оставляем, но не красим через него
  colorFrequency = 0.7,
  hoverStrength = 2,
  inertia = 0.05,
  bloom = 1.15,
  ...
  useEffect(() => {
    ...
    const fragment = /* glsl */ `
      ...
      void main(){
        ...
        // базовая яркость/насыщенность
        float L = dot(col, vec3(0.2126, 0.7152, 0.0722));
        col = clamp(mix(vec3(L), col, uSaturation), 0.0, 1.0);

        // 🔵 МАПИМ ТОЛЬКО В СИНЕ-ГОЛУБОЙ (ПОТЕМНЕЕ)
        float intens = clamp(length(col), 0.0, 1.6);
        float t = smoothstep(0.0, 1.0, intens);

        // ещё более тёмный синий низ
        vec3 deepBlue = vec3(0.03, 0.05, 0.11);   // было 0.06,0.09,0.16
        // потемневший голубой верх
        vec3 cyanBlue = vec3(0.15, 0.60, 0.92);   // было 0.22,0.74,0.97

        col = mix(deepBlue, cyanBlue, t);
    ...
    return () => {
      ...
      if (suspendWhenOffscreen) {
        const io = (container as PrismContainer)
